...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ОкладРасчитан = РасчитатьОсновныеНачисленияОклад();
...
Функция РасчитатьОсновныеНачисленияОклад()
...
Процедура РасчитатьУдержаниеНДФЛ() 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(ВКМ_УдержаниеБазаВКМ_ОсновныеНачисления.РезультатБаза, 0) КАК РезультатБаза,
	               |	ВКМ_УдержаниеБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез КАК ВидРасчетаРазрез
	               |ИЗ
	               |	РегистрРасчета.ВКМ_ОсновныеНачисления КАК ВКМ_ОсновныеНачисления
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_Удержание.БазаВКМ_ОсновныеНачисления(
	               |				&Измерения,
	               |				&Измерения,
	               |				&Разрезы,
	               |				Регистратор = &Регистратор
	               |					И ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_Удержание.НДФЛ)) КАК ВКМ_УдержаниеБазаВКМ_ОсновныеНачисления
	               |		ПО ВКМ_ОсновныеНачисления.НомерСтроки = ВКМ_УдержаниеБазаВКМ_ОсновныеНачисления.НомерСтроки
	               |			И ВКМ_ОсновныеНачисления.ВидРасчета = ВКМ_УдержаниеБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез
	               |ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.РезультатБаза = 0 Тогда
			Движение.Результат = 0;
			Продолжить;
		КонецЕсли;
		
		Движение.Результат = Выборка.РезультатБаза * РазмерНДФЛ / 100;
		
		Движение.СуммаОснования = Выборка.РезультатБаза;
